...
#         self.temp_folder = os.path.join(PJ_ROOT, '_api/data/temp') # 存放临时文件
#         os.makedirs(self.temp_folder, exist_ok=True)
        
#         if self.dataset_example:
#             self.file_path = os.path.join(PJ_ROOT,'_api/data/dataset_example/dataset.zip')
#         elif self.train_log:
#             self.file_path = os.path.join(PJ_ROOT,'_api/logs/train',f'{self.seesion_id}.log')
#         else:
...
#                 temp_folder = os.path.join(PJ_ROOT, 'data', 'temp',f"temp-{self.time_stamp}-{str(uuid4())[20:]}",os.path.basename(self.file_path).split('.')[0])
#                 os.makedirs(temp_folder, exist_ok=True)
#                 # 原图
