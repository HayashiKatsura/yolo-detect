...
def plot_metrics_and_loss(experiment_names, metrics_info, loss_info, metrics_subplot_layout, loss_subplot_layout,
    ...
    for i, (metric_name, title) in enumerate(metrics_info):
        ...
        for name in experiment_names:
            ...
            data = pd.read_csv(os.path.join(name,'results.csv'))
            column_name = [col for col in data.columns if col.strip() == metric_name][0]
            plt.plot(data[column_name], label=os.path.basename(name))
        plt.xlabel('Epoch')
        plt.title(title)
        plt.legend()
    plt.tight_layout()
...
metrics_filename, loss_filename = plot_metrics_and_loss(
    experiment_names=[
        '/home/panxiang/coding/kweilx/ultralytics/_project/mydata/_results/yolov8m/best/has-spdconv', 
        '/home/panxiang/coding/kweilx/ultralytics/_project/mydata/_results/yolov8m/no-spdconv'
                    ],
