...
def process_list(input_list):
    result = []  # 用于存放最终结果
    concat_part = []  # 用于存放拼接部分
    found_png = False  # 标记是否已经遇到.png结尾的元素
    
    for item in input_list:
        if item.endswith('.png') and not found_png:
            # 当遇到第一个以.png结尾的元素时，将之前的部分拼接
            concat_part.append(item)  # 将.png元素加到拼接部分
            result.append(' '.join(concat_part))  # 拼接部分加到结果中
            concat_part = []  # 清空拼接部分
            found_png = True  # 标记已经遇到.png元素
        elif found_png:
            # 之后的元素保持不变
            result.append(item)
        else:
            concat_part.append(item)  # 将非.png元素加入拼接部分
...
def get_random_data_with_Mosaic(annotation_line, input_shape, jitter=0.3, hue=.1, sat=0.7, val=0.4):
    ...
    for line in annotation_line:
        ...
        line_content = process_list(line_content)
        #---------------------------------#
        #   打开图片
        #---------------------------------#
