...
if __name__ == "__main__":
    ...
    for index in range(Out_Num):
        ...
        jpg_name  = os.path.join(Origin_JPEGImages_path, os.path.splitext(sample_xmls[0])[0] + '.png')
        xml_name  = os.path.join(Origin_Annotations_path, sample_xmls[0])
            
        # line = convert_annotation(jpg_name, xml_name, unique_labels)
        line = convert_annotation(jpg_name, xml_name)
        ...
        img.save(os.path.join(Out_JPEGImages_path, f'class_{os.path.basename(Origin_Annotations_path)}_random_{index+100}.png'))
        convert_to_yolo_format_and_save(
            box_data,
            os.path.join(Out_Annotations_path, f'class_{os.path.basename(Origin_Annotations_path)}_random_{index+100}.txt'),
        )