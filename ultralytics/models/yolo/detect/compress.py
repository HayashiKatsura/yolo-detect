...
def get_pruner(opt, model, example_inputs):
    """
    配置模型剪枝的策略，通过移除不重要的权重来压缩神经网络模型，从而减少模型大小和推理时间，同时尽量保持模型性能。
    最终都产生稀疏模型
    模型剪枝（这段代码）：通常在训练完成后执行，移除已训练好的权重
    稀疏化训练：在训练过程中就引入稀疏性约束
    
    问题：
    当采用稀疏化训练后，模型性能严重下架，几近腰斩，不进行稀疏化训练，模型不仅实现了剪枝反而性能还有小幅度提升
    稀疏约束在训练早期就限制了模型的学习能力
    模型可能还没学会基础特征就被强制稀疏化
    梯度流被过早截断，影响深层网络的训练
    ...
    if opt.prune_method == "random":
        ...
        """
        LAMP 是层自适应的，每层根据自己的特点决定剪枝比例
        比全局统一剪枝更精准
        考虑了不同层的重要性差异
        p=2 表示使用 L2 范数来计算重要性分数，评估器用于确定神经网络中哪些权重或结构是重要的，哪些可以被剪枝掉
        
        使用 partial 创建一个偏函数，预设了 MagnitudePruner 的部分参数
        MagnitudePruner 是基于权重幅度的剪枝器，会移除幅度较小的权重
        global_pruning=opt.global_pruning 设置是否使用全局剪枝策略
        如果为 True：在整个模型范围内比较和剪枝权重
        如果为 False：在每一层内部独立进行剪
        
        论文可以参考的点：
        揭示稀疏化训练的内在困难，为后续研究提供理论基础（稀疏约束对梯度流的影响机制，
        不同网络层对稀疏约束的敏感性差异，稀疏化训练中的优化困难根源，BN层在稀疏化过程中的行为变化）
        """
    ...
    # for yolov12-convnextv2-gatedblockx4.yaml
    from ultralytics.nn.extra_modules.GatedCNNBlockBCHW import GatedCNNBlockBCHW
    customized_pruners[LayerNorm] = LayerNormPruner()  # <- OK
    for k, m in model.named_modules():
        if isinstance(m, Detect): # 默认检测头
            ignored_layers.append(m.cv2[0][2])
            ignored_layers.append(m.cv2[1][2])
            ignored_layers.append(m.cv2[2][2])
            ignored_layers.append(m.cv3[0][2])
        ...
        if isinstance(m, AAttn):
            ignored_layers.append(m)
        # if isinstance(m, GatedCNNBlockBCHW): 
        #     ignored_layers.append(m)
...
class DetectionCompressor(BaseTrainer):
    ...
    def model_prune(self, imp, prune, example_inputs):
        """
        渐进式剪枝：逐步删除不重要的连接/神经元，而非一次性大幅剪枝
        性能监控：实时追踪模型压缩率和精度损失
        灵活的重要性评估：支持不同的重要性计算方法
        自适应停止：当达到目标加速比或最大迭代数时停止
        HessianImportance 和 TaylorImportance 用来量化神经网络中每个参数（权重、神经元、通道等）重要性
        计算重要性分数 → 使用Hessian/Taylor方法
        排序参数 → 按重要性从低到高排列
        选择剪枝目标 → 移除重要性最低的参数
        执行剪枝 → 物理删除这些参数
        微调恢复 → 训练剩余参数补偿性能损失
        ...
        imp = tp.importance.LAMPImportance(p=2)
        基于权重幅度的重要性评估，不需要计算梯度，直接使用权重值，公式：重要性 = |w|^p（这里p=2，即权重平方），按层自适应调整剪枝比例
        当使用LAMPImportance时：
        跳过梯度计算：不需要前向/反向传播
        直接基于权重：重要性 = |权重|^2
        更高效：无需数据就能评估重要性
        按层自适应：每层根据权重分布动态调整剪枝比例
        跳过不必要的前向传播计算，直接基于权重幅度进行剪枝，效率更高（不需要处理训练数据）
        """
        N_batchs = 10

        base_model = deepcopy(self.model) # 备份原始模型
        with HiddenPrints():
            try:
                # 计算原始模型的FLOPs和参数量
        ...
        ori_result = self.validate_prune() # 验证原始模型性能
        ori_map50, ori_map = ori_result['metrics/mAP50(B)'], ori_result['metrics/mAP50-95(B)'] # 获取mAP指标
    ...
    def sparsity_learning(self, ckpt, world_size, prune):
        ...
        self.batch_size = self.batch_size // 2 # 减小batch size以适应稀疏化训练的内存需求，通过梯度累积保持有效batch size，按比例调整权重衰减
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                if isinstance(prune, (tp.pruner.BNScalePruner,)):
                    if self.args.reg_decay_type == 'linear':#线性衰减：正则化强度随epoch线性变化；
                        reg = linear_trans(epoch, self.epochs, self.args.reg, self.args.reg * self.args.reg_decay)
                    elif self.args.reg_decay_type == 'step':# 阶梯衰减：按固定步长衰减正则化强度；
                        reg = self.args.reg * (self.args.reg_decay ** (epoch // self.args.reg_decay_step))
                    elif self.args.opt.reg_decay_type == 'constant':# 常数：保持恒定正则化强度
                        reg = self.args.reg
    ...
    def compress(self):
        ...
        if sparsity_learning and not self.args.sl_model:
            self.args.sl_model = self.sparsity_learning(ckpt, world_size, prune) # 稀疏训练
