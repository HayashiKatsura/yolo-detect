...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    if verbose:
        ...
        lgu.info(s if newline else s.rstrip())
...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    if detailed:
        ...
        lgu.info(f"{'layer':>5} {'name':>40} {'gradient':>9} {'parameters':>12} {'shape':>20} {'mu':>10} {'sigma':>10}")
    ...
    lgu.info(f"summary{fused}: {n_l:,} layers, {n_p:,} parameters, {n_g:,} gradients{fs}")
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    lgu.info(f"FP16|Optimizer stripped from {f},{f' saved as {s},' if s else ''} {mb_fp16:.1f}MB")
    if x_model_fp32:
        ...
        lgu.info(f"FP32|Optimizer stripped from {f},{f' saved as {s},' if s else ''} {mb_fp32:.1f}MB")
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                ...
                lgu.info(f"{p:12}{flops:12.4g}{mem:>14.3f}{tf:14.4g}{tb:14.4g}{str(s_in):>24s}{str(s_out):>24s}")
