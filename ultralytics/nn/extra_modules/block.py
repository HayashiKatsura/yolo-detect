...
from .GatedCNNBlockBCHW import GatedCNNBlockBCHW
...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_ContextGuided', 'C3k2_MSBlock', 'ContextGuidedBlock_Down', 'C3k2_DLKA', 'CSPStage', 'SPDConv','MultiScaleConv',
           'TripletAttention',
...
######################################## CVPR2025 GatedBlock start ########################################

class C3k_GatedBlock(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(GatedCNNBlockBCHW(c_) for _ in range(n)))

class C3k2_GatedBlock(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_GatedBlock(self.c, self.c, n, shortcut, g) if c3k else GatedCNNBlockBCHW(self.c) for _ in range(n))

######################################## CVPR2025 GatedBlock end ########################################
...
class DropBlock2D(nn.Module):
    ...
    def forward(self, x):
        ...
        # 随机丢弃位置
        mask_prob = self.drop_prob / (self.block_size ** 2)
        rand_tensor = torch.rand(mask_shape, device=x.device)
        mask = torch.where(rand_tensor < mask_prob, torch.zeros_like(mask), mask)
        
        # 将丢弃区域扩展到周围区域
        block_mask = F.max_pool2d(mask, kernel_size=self.block_size, stride=1, padding=self.block_size//2)
        
        # 缩放输出
        count = block_mask.numel() / block_mask.sum()
        return x * block_mask * count
        
class SPDConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, drop_prob=0.1):
        super().__init__()
        c1_spd = c1 * 4
        self.conv = nn.Conv2d(c1_spd, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
...
MutilScaleEdgeInformationEnhance_bins = [3, 6, 9, 12]
# MutilScaleEdgeInformationEnhance_bins = [3, 5, 7, 9]
class C3k_MutilScaleEdgeInformationEnhance(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(MutilScaleEdgeInformationEnhance(c_, MutilScaleEdgeInformationEnhance_bins) for _ in range(n)))

class C3k2_MutilScaleEdgeInformationEnhance(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_MutilScaleEdgeInformationEnhance(self.c, self.c, 2, shortcut, g) if c3k else MutilScaleEdgeInformationEnhance(self.c, MutilScaleEdgeInformationEnhance_bins) for _ in range(n))
